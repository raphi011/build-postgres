...
var golden = []struct{ src, want string }{
	...
	{"explain (costs off) select 1", "EXPLAIN (COSTS OFF) SELECT 1"},
	{"EXPLAIN (COSTS OFF) UPDATE t SET a = 1", "EXPLAIN (COSTS OFF) UPDATE t SET a = 1"},
	{"explain (costs on) select 1", "EXPLAIN SELECT 1"},
	{"explain (costs) select 1", "EXPLAIN SELECT 1"},
	// ANALYZE
	{"analyze", "ANALYZE"},
	{"analyze t", "ANALYZE t"},
	{`ANALYZE "T"`, `ANALYZE "T"`},
...
func TestSyntaxErrors(t *testing.T) {
	cases := []struct {
		...
		{"explain (verbose) select 1", at(9, 1, 10), "COSTS", `"verbose"`},
		{"explain () select 1", at(9, 1, 10), "COSTS", `")"`},
		{"explain (costs off select 1", at(19, 1, 20), ")", `"select"`},
		{"explain (costs off) create table t (a int4)", at(20, 1, 21), "SELECT, INSERT, UPDATE, or DELETE", `"create"`},
		{"explain analyze select 1", at(8, 1, 9), "SELECT, INSERT, UPDATE, or DELETE", `"analyze"`},
		{"analyze 1", at(8, 1, 9), "end of statement", `"1"`},
		{"analyze t u", at(10, 1, 11), "end of statement", `"u"`},
